...
try:
    # --- Load historical datasets (2020-2024) ---
    ...
    match_data = pd.read_csv("match_data.csv") # Added from R script section 6

    # --- Load and standardize 2025 Team-wise performance files ---
    # Batting
...
# R's bind_rows is equivalent to pandas' concat
all_batting_dfs = [
...
# Deduplicate and summarize scores (equivalent to group_by %>% summarise)
...
# Merge scores (equivalent to full_join)
...
# Combine team columns from the outer join
...
# Calculate total score and define roles (equivalent to mutate and case_when)
...
# Add Wicketkeeper identification from dismissals data
...
# Convert is_wicketkeeper to role if needed
def update_role(row):
...
if missing_vars:
    ...
    if user_team1 == user_team2:
        ...
        if not all(any((team['Role'] == r) & (team['team'].isin([user_team1, user_team2]))) for r in roles_needed):
            ...
            st.subheader(f"Fantasy XI for {user_team1} vs {user_team2} at {user_venue}")
            st.dataframe(team[['Player', 'team', 'Role', 'fantasy_score', 'consistency_index', 'historical_score', 'combined_score', 'Multiplier', 'final_score']])
